...
function tebuto_ajax_seminar_occurrence_action(): void {
	...
	$result = tebuto_ajax_run_occurrence_action( $api, $action, $occurrence_id, $seminar_id );
...
/**
 * Dispatch a seminar occurrence AJAX action.
 *
 ...
 */
function tebuto_ajax_run_occurrence_action( Tebuto_API $api, string $action, int $occurrence_id, int $seminar_id ) {
	switch ( $action ) {
		case 'publish':
			return tebuto_ajax_run_occurrence_publish( $api, $occurrence_id );
		case 'unpublish':
			return tebuto_ajax_run_occurrence_unpublish( $api, $occurrence_id );
		case 'cancel':
			return tebuto_ajax_run_occurrence_cancel( $api, $occurrence_id );
		case 'reorder':
			return tebuto_ajax_run_occurrence_reorder( $api, $seminar_id );
		default:
...
/**
 * Publish an occurrence via AJAX.
 *
 * @param Tebuto_API $api           API instance.
 * @param int        $occurrence_id Occurrence ID.
 * @return array|WP_Error
 */
function tebuto_ajax_run_occurrence_publish( Tebuto_API $api, int $occurrence_id ) {
	if ( $occurrence_id < 1 ) {
		wp_send_json_error( __( 'Fehlende Parameter.', 'tebuto-online-terminbuchung' ), 400 );
	}
	return $api->set_seminar_occurrence_status( $occurrence_id, 'published' );
}

/**
 * Unpublish an occurrence via AJAX.
 *
 * @param Tebuto_API $api           API instance.
 * @param int        $occurrence_id Occurrence ID.
 * @return array|WP_Error
 */
function tebuto_ajax_run_occurrence_unpublish( Tebuto_API $api, int $occurrence_id ) {
	if ( $occurrence_id < 1 ) {
		wp_send_json_error( __( 'Fehlende Parameter.', 'tebuto-online-terminbuchung' ), 400 );
	}
	return $api->set_seminar_occurrence_status( $occurrence_id, 'draft' );
}

/**
 * Cancel an occurrence via AJAX.
 *
 * @param Tebuto_API $api           API instance.
 * @param int        $occurrence_id Occurrence ID.
 * @return array|WP_Error
 */
function tebuto_ajax_run_occurrence_cancel( Tebuto_API $api, int $occurrence_id ) {
	if ( $occurrence_id < 1 ) {
		wp_send_json_error( __( 'Fehlende Parameter.', 'tebuto-online-terminbuchung' ), 400 );
	}
	// phpcs:ignore WordPress.Security.NonceVerification.Missing -- Verified by tebuto_ajax_seminar_occurrence_action().
	$reason = isset( $_POST['reason'] ) ? sanitize_textarea_field( wp_unslash( $_POST['reason'] ) ) : '';
	return $api->cancel_seminar_occurrence( $occurrence_id, $reason !== '' ? $reason : null );
}

/**
 * Reorder occurrences via AJAX.
 *
 * @param Tebuto_API $api        API instance.
 * @param int        $seminar_id Seminar ID.
 * @return array|WP_Error
 */
function tebuto_ajax_run_occurrence_reorder( Tebuto_API $api, int $seminar_id ) {
	// phpcs:disable WordPress.Security.NonceVerification.Missing -- Verified by tebuto_ajax_seminar_occurrence_action().
	...
	return $api->reorder_seminar_occurrences( $seminar_id, $ids );
}
...
function tebuto_transform_events_for_calendar( array $events_result, int $category_filter, string $status_filter ): array {
	...
	if ( ! empty( $events_result['rules'] ) ) {
		foreach ( $events_result['rules'] as $event ) {
			$row = tebuto_calendar_event_from_rule( $event, $category_filter, $status_filter );
			if ( $row !== null ) {
				$calendar_events[] = $row;
			}
		}
	...
	if ( ! empty( $events_result['standalone'] ) ) {
		foreach ( $events_result['standalone'] as $event ) {
			$row = tebuto_calendar_event_from_standalone( $event, $category_filter, $status_filter );
			if ( $row !== null ) {
				$calendar_events[] = $row;
			}
		}
...
/**
 * Extract booking client display name from details.
 *
 * @param array $details Event details.
 * @return string|null
 */
function tebuto_extract_booking_client_name( array $details ): ?string {
...
/**
 * Map a rule-based API event to a FullCalendar row, or null if filtered out.
 *
 * @param array  $event           Rule event.
 * @param int    $category_filter Category ID filter.
 * @param string $status_filter   Status filter.
 * @return array|null
 */
function tebuto_calendar_event_from_rule( array $event, int $category_filter, string $status_filter ): ?array {
	...
	if ( ! empty( $event['details'] ) ) {
		...
		$client_name = tebuto_extract_booking_client_name( $details );
	}
	...
	$color = tebuto_get_event_color( $status, $event['color'] ?? TEBUTO_COLOR_FALLBACK );
...
/**
 * Map a standalone API event to a FullCalendar row, or null if filtered out.
 *
 * @param array  $event           Standalone event.
 * @param int    $category_filter Category ID filter.
 * @param string $status_filter   Status filter.
 * @return array|null
 */
function tebuto_calendar_event_from_standalone( array $event, int $category_filter, string $status_filter ): ?array {
	if ( $category_filter > 0 && ( $event['categoryId'] ?? 0 ) !== $category_filter ) {
		return null;
	}
	...
	$client_name = ! empty( $event['details'] ) ? tebuto_extract_booking_client_name( $event['details'] ) : null;
...
/**
 * Verify calendar or admin AJAX nonce.
 *
 * @return void
 */
function tebuto_ajax_verify_calendar_or_admin_nonce(): void {
	$valid_nonce = false;
...
function tebuto_ajax_booking_action(): void {
	tebuto_ajax_verify_calendar_or_admin_nonce();
	...
	// phpcs:disable WordPress.Security.NonceVerification.Missing -- Verified by tebuto_ajax_verify_calendar_or_admin_nonce().
	...
	$result = tebuto_ajax_run_booking_action( $api, $action, $booking_id );
...
/**
 * Dispatch a booking AJAX action.
 *
 * @param Tebuto_API $api        API instance.
 * @param string     $action     Action name.
 ...
 * @return array|WP_Error
 */
function tebuto_ajax_run_booking_action( Tebuto_API $api, string $action, int $booking_id ) {
	// phpcs:disable WordPress.Security.NonceVerification.Missing -- Verified by tebuto_ajax_booking_action().
	switch ( $action ) {
		case 'confirm':
			return $api->confirm_booking( $booking_id );
		case 'reject':
			$message = isset( $_POST['message'] ) ? sanitize_textarea_field( wp_unslash( $_POST['message'] ) ) : '';
			return $api->reject_booking( $booking_id, $message );
		case 'cancel':
			$message        = isset( $_POST['message'] ) ? sanitize_textarea_field( wp_unslash( $_POST['message'] ) ) : '';
			$bookable_again = isset( $_POST['bookable_again'] ) ? (bool) $_POST['bookable_again'] : true;
			return $api->cancel_booking( $booking_id, $message, $bookable_again );
		default:
			wp_send_json_error( __( 'Unbekannte Aktion.', 'tebuto-online-terminbuchung' ), 400 );
	}
	// phpcs:enable WordPress.Security.NonceVerification.Missing
