	...
	if ( ! empty( $events_result['rules'] ) ) {
		foreach ( $events_result['rules'] as $event ) {
			...
			$calendar_events[] = array(
				...
				'start'           => $event['start'],
				'end'             => $event['end'],
				'backgroundColor' => tebuto_get_event_color( $status, $event['color'] ?? TEBUTO_COLOR_FALLBACK ),
				'borderColor'     => tebuto_get_event_color( $status, $event['color'] ?? TEBUTO_COLOR_FALLBACK ),
