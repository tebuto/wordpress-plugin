...
export default function Edit({ attributes, setAttributes }) {
	...
	const selectedCategories = useMemo(
		() => (categories ? categories.split(',').map((id) => parseInt(id.trim(), 10)) : []),
		[categories]
	)
					...
					if (errorCode === 'session_expired') {
						...
						const errorMessage =
							typeof errorPayload === 'object' && errorPayload !== null && errorPayload.message
								...
								: errorPayload
						setCategoriesError(
							errorMessage || __('Kategorien konnten nicht geladen werden.', 'tebuto-online-terminbuchung')
						)
					}
				}
			} catch {
				setCategoriesError(__('Verbindungsfehler beim Laden der Kategorien.', 'tebuto-online-terminbuchung'))
			} finally {
	...
	const loadWidgetPreview = useCallback(() => {
		...
		// Clear container
		previewContainerRef.current.innerHTML = '<div id="tebuto-booking-widget"></div>'
			...
			<InspectorControls>
				...
				<PanelBody title={__('Kategorien', 'tebuto-online-terminbuchung')} initialOpen={true}>
					<ToggleControl
						...
						checked={selectedAvailableCategories.length > 1 && showCategorySelectionFirst !== false}
					...
					{categoriesError && <p className="tebuto-error">{categoriesError}</p>}
						...
						<div className="tebuto-category-list">
							{availableCategories.length === 0 ? (
								...
								availableCategories.map((cat) => {
									const isSelectable = isCategoryWidgetSelectable(cat)
									...
									return (
										...
										>
											<CheckboxControl
												...
												checked={isSelectable && selectedCategories.includes(cat.id)}
