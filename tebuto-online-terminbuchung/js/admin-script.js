...
(function ($) {
    ...
    function showBookingModal(booking) {
        ...
        html += '<p><strong>' + (tebutoAdmin?.strings?.name || 'Name') + ':</strong> ' + 
            escapeHtml(booking.client.firstName + ' ' + booking.client.lastName) + '</p>';
        
        if (booking.client.email) {
            html += '<p><strong>E-Mail:</strong> ' + escapeHtml(booking.client.email) + '</p>';
        }
        if (booking.client.phoneNumber) {
            html += '<p><strong>' + (tebutoAdmin?.strings?.phone || 'Telefon') + ':</strong> ' + 
                escapeHtml(booking.client.phoneNumber) + '</p>';
        }
        html += '</div>';
        ...
        if (booking.event.category) {
            html += '<p><strong>' + (tebutoAdmin?.strings?.category || 'Kategorie') + ':</strong> ' + 
                escapeHtml(booking.event.category.name) + '</p>';
        }
        ...
        const locationStr = booking.locationSelection === 'virtual' ? 
        ...
        html += '<p><strong>' + (tebutoAdmin?.strings?.bookedOn || 'Gebucht am') + ':</strong> ' + 
            createdAt.toLocaleDateString('de-DE') + ' ' + createdAt.toLocaleTimeString('de-DE') + '</p>';

        html += '<p><strong>' + (tebutoAdmin?.strings?.price || 'Preis') + ':</strong> ' + 
            parseFloat(booking.price).toFixed(2).replace('.', ',') + ' €</p>';

        if (booking.isConfirmed) {
            html += '<p><span class="tebuto-badge tebuto-badge-success">' + 
                (tebutoAdmin?.strings?.confirmed || 'Bestätigt') + '</span></p>';
        }
