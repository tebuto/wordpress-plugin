...
export default function Edit({ attributes, setAttributes }) {
		...
		const fetchCategories = async () => {
			try {
				...
				if (data.success) {
					...
					setCategoriesError(
						data.data ||
							__("Kategorien konnten nicht geladen werden.", "tebuto-online-terminbuchung")
					);
				}
			} catch (error) {
				setCategoriesError(
					__("Verbindungsfehler beim Laden der Kategorien.", "tebuto-online-terminbuchung")
				);
			} finally {
	...
	// When provider filter is active, do NOT store configured-categories.
	// The external widget breaks its therapist dropdown when
	// configured-categories is present alongside include-subusers.
	// When provider filter is off, store categories for the widget dropdown.
	useEffect(() => {
	...
	}, [showProviderFilter, availableCategories]);
	...
	const loadWidgetPreview = () => {
		if (!previewContainerRef.current || !therapistUUID || !widgetUrl) return;
		...
		if (showProviderFilter) {
			...
			// filter is active — the widget breaks its therapist dropdown
			// when this attribute is present alongside include-subusers.
		}
		...
		return (
			<div {...blockProps}>
				<div className="tebuto-block-notice">
					<p>
						<strong>
							{__("Tebuto nicht verbunden", "tebuto-online-terminbuchung")}
						</strong>
					</p>
					<p>
						{__(
	...
	return (
		<>
			<InspectorControls>
				{/* Theme Presets */}
				<PanelBody
					title={__("Farbvorlagen", "tebuto-online-terminbuchung")}
					initialOpen={true}
				>
				...
				>
					<div className="tebuto-color-control">
						<label className="tebuto-color-label">
							{__("Primärfarbe", "tebuto-online-terminbuchung")}
						</label>
						<p className="tebuto-color-description">
							{__("Buttons und Akzente", "tebuto-online-terminbuchung")}
						</p>
						<ColorPicker
							color={primaryColor}
					...
					<div className="tebuto-color-control">
						<label className="tebuto-color-label">
							{__("Hintergrund", "tebuto-online-terminbuchung")}
						</label>
						<p className="tebuto-color-description">
							{__("Widget-Hintergrund", "tebuto-online-terminbuchung")}
						</p>
						<ColorPicker
					...
					<div className="tebuto-color-control">
						<label className="tebuto-color-label">
							{__("Sekundärtext", "tebuto-online-terminbuchung")}
						</label>
						<p className="tebuto-color-description">
							{__("Beschreibungen", "tebuto-online-terminbuchung")}
						</p>
						<ColorPicker
							color={textSecondary}
					...
					<div className="tebuto-color-control">
						<label className="tebuto-color-label">
							{__("Rahmenfarbe", "tebuto-online-terminbuchung")}
						</label>
						<p className="tebuto-color-description">
							{__("Rahmen und Trennlinien", "tebuto-online-terminbuchung")}
						</p>
						<ColorPicker
							color={borderColor}
				...
				{/* Display Options */}
				<PanelBody
					title={__("Anzeige-Optionen", "tebuto-online-terminbuchung")}
					initialOpen={false}
				>
					<ToggleControl
					...
					{isMultiUser && (
						<ToggleControl
							label={__("Schnellfilter anzeigen", "tebuto-online-terminbuchung")}
							help={__(
				...
				>
					...
					{loadingCategories && (
						<div className="tebuto-loading">
							<Spinner />
							<span>
								{__("Kategorien werden geladen...", "tebuto-online-terminbuchung")}
							</span>
						</div>
					)}
						...
						<div className="tebuto-category-list">
							{availableCategories.length === 0 ? (
								<p className="tebuto-empty">
									{__("Keine Kategorien vorhanden.", "tebuto-online-terminbuchung")}
								</p>
							) : (
								availableCategories.map((cat) => (
									<div key={cat.id} className="tebuto-category-item">
										<CheckboxControl
											label={
